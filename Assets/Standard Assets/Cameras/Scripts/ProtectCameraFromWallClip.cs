...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        Transform m_Cam;                  // the transform of the camera
        Transform m_Pivot;                // the point at which the camera pivots around
        float m_OriginalDist;             // the original distance to the camera before any modification are made
        float m_MoveVelocity;             // the velocity at which the camera moved
        float m_CurrentDist;              // the current distance from the camera to the target
        Ray m_Ray;                        // the ray used in the lateupdate for casting between the camera and the target
        RaycastHit[] m_Hits;              // the hits between the camera and the target
        RayHitComparer m_RayHitComparer;  // variable to compare raycast hit distances
